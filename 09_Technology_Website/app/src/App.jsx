...
import {
  ShoppingBag, Plus, Minus, X, Music2, Pause, Star,
  Coffee, Sparkles, Leaf, ArrowRight, MapPin
} from 'lucide-react'
...
const rise = {
  ...
  show: { opacity: 1, y: 0, transition: { duration: .6, ease: [.2, .7, .2, 1] } },
}
const Reveal = ({ children, className = '', delay = 0 }) => (
  <motion.div className={className} variants={rise} initial="hidden"
    whileInView="show" viewport={{ once: true, amount: .2 }}
    transition={{ delay }}>{children}</motion.div>
)
      ...
      {Object.entries(data).map(([k, v]) => (
        <div key={k} className="flex items-center gap-2 text-[11px] text-mute">
          ...
          <span className="flex gap-[3px]">
            {[1, 2, 3, 4, 5].map(i => (
              <i key={i} className="w-[15px] h-[7px] rounded-[2px]"
                 style={{ background: i <= v ? 'var(--color-caramel)' : 'rgba(44,32,24,.12)' }} />
            ))}
          </span>
        </div>
      ))}
  ...
  return (
    <Reveal className="group flex flex-col rounded-3xl bg-foam border border-latte/70 overflow-hidden shadow-[0_10px_30px_-18px_rgba(70,46,28,.3)] hover:shadow-[0_22px_50px_-22px_rgba(70,46,28,.4)] hover:-translate-y-2 transition-all duration-300">
      <div className="relative aspect-square overflow-hidden">
        ...
        <span className="absolute top-3 left-3 text-[11px] font-bold px-3 py-1 rounded-full backdrop-blur"
              style={{ background: p.premium ? 'var(--color-gold)' : 'rgba(255,255,255,.85)', color: p.premium ? '#fff' : 'var(--color-espresso)' }}>
          {p.bestseller ? '★ Bestseller' : p.tag}
        </span>
      </div>
      <div className="p-5 flex flex-col gap-1 flex-1">
        <div className="font-serif text-caramel text-base leading-none">{p.arabic}</div>
        ...
        <div className="flex gap-1.5 mt-1">
          {SIZES.map(s => (
            <button key={s} onClick={() => setSize(s)}
              className={`flex-1 rounded-xl py-1.5 text-xs font-semibold border transition ${size === s ? 'bg-espresso text-cream border-espresso' : 'border-latte text-mute hover:border-caramel'}`}>
              {s}
            </button>
          ))}
        </div>
        <div className="mt-auto pt-3 flex items-center justify-between">
          ...
          <button onClick={() => onAdd(p, size)}
            className="inline-flex items-center gap-1.5 rounded-full px-4 py-2 font-semibold text-sm text-white bg-gradient-to-br from-caramel to-caramel-dark hover:shadow-lg hover:-translate-y-0.5 transition">
            <Plus size={16} /> Add
          </button>
        </div>
      </div>
...
function MusicPlayer() {
  ...
  const click = () => setOn(toggleAudio())
  return (
    <button onClick={click}
      className="fixed left-5 bottom-5 z-40 flex items-center gap-3 pl-2 pr-4 py-2 rounded-full bg-foam/90 backdrop-blur border border-latte shadow-[0_18px_50px_-22px_rgba(70,46,28,.5)] hover:border-caramel transition group">
      <span className={`w-10 h-10 rounded-full grid place-items-center text-white bg-gradient-to-br from-caramel to-caramel-dark ${on ? 'spin' : ''}`}>
        {on ? <Pause size={18} /> : <Music2 size={18} />}
      </span>
      <span className="text-left">
        <b className="block text-[13px] leading-none">{on ? 'Now playing' : 'Play café music'}</b>
        <span className="text-[11px] text-mute">oud · café ambiance</span>
      </span>
      <span className="flex items-end gap-[2px] h-4 ml-1">
        {[0, 1, 2, 3].map(i => (
          <i key={i} className={`w-[3px] rounded-full bg-caramel ${on ? 'eqbar' : ''}`}
             style={{ height: 4, animationDelay: `${i * .15}s` }} />
        ))}
      </span>
    </button>
  )
...
export default function App() {
  ...
  const add = (p, size) => {
    ...
    setCart(c => {
      ...
      return [...c, { key, id: p.id, size, name: p.name, price: p.prices[size], img: p.img, qty: 1 }]
    })
    ...
    <div className="bg-ambient min-h-screen">
      ...
      <header className="sticky top-0 z-30 backdrop-blur bg-cream/80 border-b border-latte/70">
        <div className="mx-auto w-[92vw] max-w-[1180px] flex items-center justify-between py-3.5">
          <a href="#" className="flex items-center gap-2 font-serif text-2xl font-extrabold">
            <span className="w-9 h-9 rounded-full grid place-items-center text-white bg-gradient-to-br from-caramel to-caramel-dark">ك</span>
            Kogia<span className="text-caramel">.</span>
          </a>
          <nav className="hidden md:flex gap-7 text-sm text-mute font-medium">
            <a href="#blends" className="hover:text-caramel">Blends</a>
            <a href="#shop" className="hover:text-caramel">The Shop</a>
            <a href="#promise" className="hover:text-caramel">Our Promise</a>
            <a href="#love" className="hover:text-caramel">Reviews</a>
          </nav>
          <button onClick={() => setOpen(true)}
            className="relative inline-flex items-center gap-2 rounded-full px-4 py-2 font-semibold text-sm bg-foam border border-latte hover:border-caramel transition">
      ...
      <section className="mx-auto w-[92vw] max-w-[1180px] grid md:grid-cols-2 gap-10 items-center py-16 md:py-24">
        <Reveal>
          ...
          <h1 className="font-serif text-5xl md:text-7xl leading-[1.05] mt-3 mb-4">Roasted traditions,<br/><span className="italic text-caramel">ground fresh.</span></h1>
          ...
          <div className="flex gap-3 mt-8 flex-wrap">
            <a href="#blends" className="inline-flex items-center gap-2 rounded-full px-6 py-3 font-semibold text-white bg-gradient-to-br from-caramel to-caramel-dark hover:-translate-y-0.5 hover:shadow-lg transition">Shop the blends <ArrowRight size={17}/></a>
            <a href="#shop" className="inline-flex items-center gap-2 rounded-full px-6 py-3 font-semibold bg-foam border border-latte hover:border-caramel transition">Visit the shop</a>
          </div>
          <div className="flex gap-9 mt-9">
            {[['6','blends'],['100%','listed grams'],['15 DT','from / bag']].map(([a,b])=>(
              <div key={b}><b className="font-serif text-3xl block">{a}</b><span className="text-xs uppercase tracking-wide text-mute">{b}</span></div>
            ))}
          </div>
        </Reveal>
        <Reveal delay={.1}>
      ...
      {/* Blends */}
      <section id="blends" className="mx-auto w-[92vw] max-w-[1180px] py-20">
        <Reveal className="text-center max-w-[60ch] mx-auto mb-12">
          ...
          <h2 className="font-serif text-4xl md:text-5xl mt-2">Find your blend</h2>
      ...
      {/* Experience */}
      <section id="shop" className="bg-sand/50 border-y border-latte/70 py-20">
        <div className="mx-auto w-[92vw] max-w-[1180px]">
          <Reveal className="text-center max-w-[60ch] mx-auto mb-10">
            ...
            <h2 className="font-serif text-4xl md:text-5xl mt-2">Taste before you take it home</h2>
          </Reveal>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              { ic:<ShoppingBag/>, n:'03', t:'Take the bag', d:'Loved it? The matching bag is right there — 250 g, 500 g or 1 kg, sealed fresh.' },
            ].map((c,i)=>(
              <Reveal key={i} delay={i*.08} className="rounded-3xl bg-foam border border-latte/70 p-8 hover:-translate-y-1.5 transition shadow-[0_10px_30px_-20px_rgba(70,46,28,.3)]">
                ...
                <h3 className="font-serif text-2xl mb-1">{c.t}</h3>
      ...
      {/* Promise */}
      <section id="promise" className="mx-auto w-[92vw] max-w-[1180px] py-20 grid md:grid-cols-2 gap-10 items-center">
        <Reveal>
          <div className="text-[11px] font-bold tracking-[.24em] uppercase text-caramel">Our Promise</div>
          <h2 className="font-serif text-4xl md:text-5xl mt-2 mb-4">No secrets.<br/>Just grams.</h2>
          ...
          <a href="#blends" className="inline-flex items-center gap-2 rounded-full px-6 py-3 mt-6 font-semibold text-white bg-gradient-to-br from-caramel to-caramel-dark hover:-translate-y-0.5 transition">Browse blends <ArrowRight size={17}/></a>
        </Reveal>
        <Reveal delay={.1}>
          <div className="rounded-[24px] overflow-hidden aspect-[4/3] shadow-[0_30px_70px_-30px_rgba(70,46,28,.5)]">
            <img src={PHOTOS.beans} alt="Coffee beans" className="w-full h-full object-cover"/>
          </div>
        </Reveal>
      </section>

      {/* Testimonials */}
      <section id="love" className="bg-sand/50 border-y border-latte/70 py-20">
        <div className="mx-auto w-[92vw] max-w-[1180px]">
          <Reveal className="text-center mb-10">
            <div className="text-[11px] font-bold tracking-[.24em] uppercase text-caramel">Loved in Djerba</div>
            <h2 className="font-serif text-4xl md:text-5xl mt-2">From our tasting corner</h2>
          </Reveal>
      ...
      <footer className="bg-espresso text-cream/80 py-14">
        <div className="mx-auto w-[92vw] max-w-[1180px]">
          <div className="flex justify-between gap-8 flex-wrap">
            ...
            {[['Shop',[['All blends','#blends'],['Gift box','#blends'],['Visit us','#shop']]],['Company',[['Our promise','#promise'],['Owner dashboard','owner/dashboard.html']]],['Contact',[['Djerba, Tunisia','#'],['@kogiacoffee','#']]]].map(([h,items])=>(
              <div key={h}><h4 className="font-serif text-cream mb-3">{h}</h4>{items.map(([x,href])=><a key={x} href={href} className="block text-sm text-cream/65 hover:text-gold py-0.5">{x}</a>)}</div>
            ))}
          </div>
          <div className="border-t border-cream/15 mt-9 pt-5 text-xs text-cream/50 flex justify-between flex-wrap gap-2">
            <span>© 2026 Kogia Coffee — testing build, work in progress.</span>
            <span>Roasted traditions, ground fresh.</span>
          </div>
        </div>
      </footer>
      ...
      <aside className={`fixed top-0 right-0 h-full w-[min(420px,92vw)] z-50 bg-cream border-l border-latte shadow-2xl flex flex-col transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between px-6 py-5 border-b border-latte">
          <h3 className="font-serif text-2xl">Your bag</h3>
          <button onClick={() => setOpen(false)} className="text-mute hover:text-espresso"><X/></button>
        </div>
        <div className="flex-1 overflow-y-auto thin-scroll px-6 py-5 flex flex-col gap-4">
          {cart.length === 0
            ? <div className="text-center text-mute mt-12">Your bag is empty.<br/>Go taste a blend ☕</div>
            : cart.map(i => (
              <div key={i.key} className="flex gap-3 items-center">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-mute">{i.size} · {i.price} {CURRENCY}</div>
                  <div className="flex items-center gap-2 mt-1">
                    <button onClick={() => chg(i.key, -1)} className="w-6 h-6 rounded-md border border-latte grid place-items-center"><Minus size={13}/></button>
                    <span className="text-sm">{i.qty}</span>
                    <button onClick={() => chg(i.key, 1)} className="w-6 h-6 rounded-md border border-latte grid place-items-center"><Plus size={13}/></button>
                  </div>
                </div>
        ...
        <div className="border-t border-latte px-6 py-5">
          <div className="flex justify-between mb-4 text-lg"><span>Total</span><b className="font-serif text-2xl text-caramel">{total} {CURRENCY}</b></div>
          <button onClick={checkout} className="w-full rounded-full py-3 font-semibold text-white bg-gradient-to-br from-caramel to-caramel-dark hover:-translate-y-0.5 transition">Checkout</button>
        </div>
      </aside>
