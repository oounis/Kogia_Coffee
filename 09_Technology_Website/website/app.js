...
function meterRow(label, val){
  let bars = "";
  for(let i=1;i<=5;i++) bars += `<i class="${i<=val?'on':''}"></i>`;
  ...
  grid.innerHTML = PRODUCTS.map(p=>{
    ...
    <article class="card">
      <div class="card-img" style="background-image:url('${p.img}')">
        <span class="${tagClass}">${p.bestseller?'★ Bestseller':p.tag}</span>
      </div>
      <div class="card-body">
        ...
        <div class="meter">
          ${meterRow("Body",p.meter.body)}
          ${meterRow("Spice",p.meter.spice)}
          ${meterRow("Sweet",p.meter.sweet)}
          ${meterRow("Aroma",p.meter.aroma)}
        </div>
...
function pickSize(id,size){
  ...
  document.querySelectorAll(`#sizes-${id} button`).forEach(b=>{
...
function addToCart(id){
  const size = selectedSize[id] || "250g";
  const key = `${id}_${size}`;
  const existing = cart.find(c=>c.key===key);
  if(existing){ existing.qty++; }
  else{
    const p = PRODUCTS.find(x=>x.id===id);
    cart.push({key,id,size,name:p.name,price:p.prices[size],img:p.img,qty:1});
  }
  saveCart(); renderCart();
  toast(`Added ${PRODUCTS.find(x=>x.id===id).name} (${size})`);
}
...
function renderCart(){
  ...
  if(!cart.length){ box.innerHTML = `<div class="cart-empty">Your bag is empty.<br>Go taste a blend ☕</div>`; }
    ...
    box.innerHTML = cart.map(c=>`
      <div class="cart-item">
        <div class="ci-img" style="background-image:url('${c.img}')"></div>
        <div class="ci-main">
          <b>${c.name}</b><br><span>${c.size} · ${c.price} ${CURRENCY}</span>
          <div class="qty">
        ...
        <div style="font-family:var(--serif);font-weight:700">${c.price*c.qty} ${CURRENCY}</div>
      </div>`).join("");
  }
