...
export default function Track(){
  ...
  return (<div className="bg-ambient min-h-screen"><div className="mx-auto w-[92vw] max-w-[680px] py-8">
    ...
    <div className="text-center mb-6"><div className="w-14 h-14 rounded-2xl grid place-items-center text-white mx-auto" style={{background:'#B5673A'}}><PackageSearch size={26}/></div>
    ...
    {order&&<div className="card p-6 mt-5">
      ...
      <div className="flex items-center mt-6 mb-2">{STEPS.map((s,i)=>(<div key={s} className="flex-1 flex items-center last:flex-none">
        <div className="flex flex-col items-center"><div className="w-8 h-8 rounded-full grid place-items-center text-white text-xs font-bold" style={{background:i<=stepIdx?'#B5673A':'#E5DCD2'}}>{i+1}</div><div className="text-[10px] mt-1 text-center w-16 leading-tight" style={{color:i<=stepIdx?'#2A211B':'#8B7B6E'}}>{s}</div></div>
        {i<STEPS.length-1&&<div className="flex-1 h-1 rounded-full mx-1 -mt-4" style={{background:i<stepIdx?'#B5673A':'#E5DCD2'}}/>}
      </div>))}</div>

      <div className="rounded-2xl p-4 mt-4 text-sm" style={{background:'#F6EAE0'}}>
        <div className="flex items-center gap-2 font-semibold" style={{color:'#B5673A'}}><Wallet size={16}/> {order.status==='Livrée'?'Commande livrée et réglée':`${order.total} ${CUR} à payer à la livraison`}</div>
