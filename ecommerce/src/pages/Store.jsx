...
import { ShoppingBag, Plus, Minus, X, Coffee, Truck, Leaf, ArrowRight, Wallet, Sparkles } from 'lucide-react'
import { PRODUCTS, SIZES, CUR } from '../data.js'
...
export default function Store(){
  ...
  return (<div className="bg-ambient min-h-screen">
    <header className="sticky top-0 z-30 backdrop-blur bg-cream/80 border-b border-line">
      <div className="mx-auto w-[92vw] max-w-[1120px] flex items-center justify-between py-3.5">
        <a href="#" className="flex items-center gap-2"><span className="w-9 h-9 rounded-xl grid place-items-center text-white text-lg" style={{background:'#B5673A'}}>☕</span>
          <div><div className="serif font-extrabold leading-none text-lg">Kogia Coffee</div><div className="text-[10px] text-muted">par Kogia Business</div></div></a>
        <nav className="hidden md:flex gap-7 text-sm text-muted font-medium">
          <a href="#produits" className="hover:text-caramel">Nos cafés</a><a href="#qualite" className="hover:text-caramel">Qualité</a><a href="#livraison" className="hover:text-caramel">Livraison</a>
        </nav>
        <button onClick={()=>setOpen(true)} className="relative inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold text-white" style={{background:'#B5673A'}}>
          <ShoppingBag size={16}/> Panier {count>0&&<span className="absolute -top-1.5 -right-1.5 min-w-5 h-5 px-1 grid place-items-center rounded-full text-[11px] bg-ink text-white">{count}</span>}</button>
      </div>
    </header>
    ...
    <section className="mx-auto w-[92vw] max-w-[1120px] grid md:grid-cols-2 gap-10 items-center py-16">
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}>
        <div className="inline-flex items-center gap-2 text-xs font-bold px-3 py-1 rounded-full" style={{background:'#F6EAE0',color:'#B5673A'}}><Sparkles size={13}/> Torréfié & moulu frais · Tunisie 🇹🇳</div>
        ...
        <p className="text-lg text-muted mt-4 max-w-[44ch]">Six mélanges traditionnels, en poudre, par sachet. Commandez en ligne — <b className="text-ink">paiement à la livraison</b> partout en Tunisie.</p>
        <div className="flex gap-3 mt-7 flex-wrap">
          <a href="#produits" className="inline-flex items-center gap-2 rounded-full px-6 py-3 font-semibold text-white" style={{background:'#B5673A'}}>Voir nos cafés <ArrowRight size={17}/></a>
          <a href="#livraison" className="inline-flex items-center gap-2 rounded-full px-6 py-3 font-semibold bg-white border border-line">Livraison & paiement</a>
        </div>
      </motion.div>
      <motion.div initial={{opacity:0,scale:.96}} animate={{opacity:1,scale:1}} className="relative">
        <div className="card p-8 aspect-[4/5] flex flex-col justify-between" style={{background:'linear-gradient(160deg,#F6EAE0,#EAD7C6)'}}>
          <div className="text-6xl">☕</div>
          <div><div className="serif text-2xl font-bold">Signature Djerbi</div><div className="text-muted text-sm">الجربية · aromatique & rond</div>
            <div className="mt-2 inline-block text-white text-sm font-bold px-3 py-1 rounded-full" style={{background:'#B5673A'}}>dès 16 {CUR}</div></div>
        </div>
      </motion.div>
    </section>
    ...
    <div className="border-y border-line bg-white"><div className="mx-auto w-[92vw] max-w-[1120px] flex flex-wrap gap-x-8 gap-y-2 justify-around py-3.5 text-sm text-muted">
      ...
      <span className="flex items-center gap-2"><Leaf size={16} style={{color:'#B5673A'}}/> Sans sucre ajouté</span>
    </div></div>

    {/* produits */}
    <section id="produits" className="mx-auto w-[92vw] max-w-[1120px] py-16">
      <div className="text-center max-w-[60ch] mx-auto mb-10"><div className="text-xs font-bold uppercase tracking-widest" style={{color:'#B5673A'}}>Notre carte</div>
        <h2 className="serif text-4xl font-extrabold mt-2">Trouvez votre mélange</h2><p className="text-muted mt-2">Du plus léger au plus corsé. Choisissez votre format.</p></div>
    ...
    <section id="qualite" className="bg-white border-y border-line py-16"><div className="mx-auto w-[92vw] max-w-[1120px]">
      ...
      <div className="grid md:grid-cols-3 gap-5">
        {[[<Coffee/>,'Torréfié frais','Chaque mélange est torréfié et moulu à la commande, jamais en avance.'],[<Leaf/>,'Recettes authentiques','Des recettes traditionnelles, listées au gramme près — pas de secret.'],[<Truck/>,'Livré chez vous','Commandez en ligne, recevez à domicile et payez à la livraison.']].map(([ic,t,d],i)=>(
          <div key={i} className="card p-6"><div className="w-12 h-12 rounded-xl grid place-items-center mb-3" style={{background:'#F6EAE0',color:'#B5673A'}}>{ic}</div><h3 className="serif text-xl font-bold">{t}</h3><p className="text-muted text-sm mt-1">{d}</p></div>))}
      </div>
    </div></section>
    ...
    <section id="livraison" className="mx-auto w-[92vw] max-w-[1120px] py-16">
      <div className="card p-8 md:p-10 grid md:grid-cols-[1.2fr_.8fr] gap-6 items-center" style={{background:'linear-gradient(135deg,#fff,#F6EAE0)'}}>
        <div><div className="inline-flex items-center gap-2 text-xs font-bold px-3 py-1 rounded-full mb-3" style={{background:'#B5673A',color:'#fff'}}><Wallet size={13}/> Paiement à la livraison</div>
          ...
          <p className="text-muted mt-2 max-w-[48ch]">Pas de paiement en ligne. Commandez, et réglez en espèces au livreur quand vous recevez votre café. Livraison dans les 24 gouvernorats de Tunisie sous 2 à 4 jours.</p>
    ...
    <footer className="bg-ink text-white/75 py-12"><div className="mx-auto w-[92vw] max-w-[1120px] flex flex-wrap justify-between gap-6">
      <div className="max-w-[280px]"><div className="serif font-extrabold text-white text-lg">Kogia Coffee</div><p className="text-sm mt-1">Café traditionnel tunisien, torréfié & moulu frais. Une marque de Kogia Business.</p></div>
    ...
    <aside className={`fixed top-0 right-0 h-full w-[min(420px,92vw)] z-50 bg-cream border-l border-line flex flex-col transition-transform duration-300 ${open?'translate-x-0':'translate-x-full'}`}>
      ...
      <div className="flex-1 overflow-y-auto thin px-6 py-5 flex flex-col gap-4">
        {cart.length===0? <div className="text-center text-muted mt-12">Votre panier est vide.<br/>Choisissez un café ☕</div>
          : cart.map(i=>(<div key={i.key} className="flex gap-3 items-center"><div className="w-12 h-12 rounded-xl grid place-items-center text-xl" style={{background:'#F6EAE0'}}>☕</div>
      ...
      <div className="border-t border-line px-6 py-5"><div className="flex justify-between mb-3 text-lg"><span>Total</span><b className="serif text-2xl" style={{color:'#B5673A'}}>{total} {CUR}</b></div>
...
function ProductCard({p,onAdd}){
  ...
  return (<motion.div initial={{opacity:0,y:16}} whileInView={{opacity:1,y:0}} viewport={{once:true}} className="card overflow-hidden flex flex-col hover:shadow-xl transition">
    <div className="aspect-[5/3] grid place-items-center text-5xl relative" style={{background:`linear-gradient(140deg,${p.c1},${p.c2})`}}>☕
      <span className="absolute top-3 left-3 text-[11px] font-bold px-2.5 py-1 rounded-full" style={{background:p.premium?'#B5673A':'rgba(255,255,255,.85)',color:p.premium?'#fff':'#2A211B'}}>{p.best?'★ Best-seller':p.tag}</span></div>
    <div className="p-5 flex flex-col gap-1 flex-1">
      <div className="serif text-lg" style={{color:'#B5673A'}}>{p.ar}</div>
      ...
      <div className="flex items-center justify-between mt-auto pt-3"><div className="serif text-xl font-bold">{p.prices[size]} <span className="text-xs text-muted">{CUR}</span></div>
        <button onClick={()=>onAdd(p,size)} className="inline-flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-semibold text-white" style={{background:'#B5673A'}}><Plus size={15}/> Ajouter</button></div>
    </div>
  </motion.div>)
}
